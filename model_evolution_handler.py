...
def _gemini_generate_text(state, prompt: str) -> str:
    ...
    if gemini is None:
        raise RuntimeError("AndyAI Gemini handle not found in state.")
    ...
    raise RuntimeError("Gemini handle does not expose generate_text() or __call__().")
